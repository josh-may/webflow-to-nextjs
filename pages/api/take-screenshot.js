...
export default async function handler(req, res) {
  ...
  const { cssSelector, url } = req.body;

  if (!cssSelector || !url) {
    console.log("❌ CSS selector or URL is missing");
    return res
      .status(400)
      .json({ error: "CSS selector and URL are required." });
  }
  ...
  try {
    ...
    const response = await getScreenshotWithRetry(scrapingBeeClient, {
      ...
      params: {
        screenshot: true,
        screenshot_selector: cssSelector,
        screenshot_full_page: false,
        render_js: true,
        json_response: true,
        block_resources: false,
        wait_for: cssSelector, // Add wait for selector
